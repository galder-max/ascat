...
ascat.predictGermlineGenotypes <- function (ASCATobj, platform = "AffySNP6", img.dir = ".", img.prefix = "")
{
  ...
  failedarrays = NULL
    for (i in 1:dim(ASCATobj$Tumor_LogR)[2]) {
        ...
        if(platform=="HTC_30X+")
        {
            sds.bsm <- sd(diff(bsm[bsm<.85&bsm>.15]))
            print(sds.bsm)
            if(sds.bsm>.1)
                homoLimit <- .15
            else if(sds.bsm>.09)
        ...
        png(filename = file.path(img.dir, paste(img.prefix, "tumorSep.",sds.bsm,".",
